...
Questo progetto implementa un sistema di **Retrieval-Augmented Generation (RAG)** completamente locale. L'obiettivo è permettere agli utenti di interrogare i propri documenti (PDF, Markdown, TXT) utilizzando modelli linguistici di grandi dimensioni (LLM), garantendo la privacy dei dati e l'assenza di costi di API esterne.
...
2. **Embedding & Vector Storage**: Trasformazione dei chunk in vettori densi tramite il modello `nomic-embed-text` e memorizzazione in **ChromaDB**.
3. **RAG Cycle**:
